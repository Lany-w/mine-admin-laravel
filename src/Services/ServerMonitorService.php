...
{
    private string $sysctlPath = '';
    private string $systemProfilerPath = '';
    public function __construct()
    {
        if (!is_executable('sysctl')) {
            $this->sysctlPath = '/usr/sbin/';
        }
        if (!is_executable('system_profiler')) {
            $this->systemProfilerPath = '/usr/sbin/';
        }
    }
    public function isCygwin(): bool
    {
    ...
    public function getCpuInfo(): array
    {
        try {
            if (PHP_OS == 'Linux') {
                ...
                $cpu = $cpu[1] ?? '未知';
                preg_match('/(\d+)/', shell_exec($this->sysctlPath.'sysctl -n hw.l2cachesize'), $cache);
    ...
    public function getCpuName(): string
    {
        ...
        return shell_exec($this->sysctlPath.'sysctl -n machdep.cpu.brand_string');
    }
    ...
    public function getCpuPhysicsCores(): string
    {
        ...
        return trim(shell_exec($this->sysctlPath.'sysctl -n hw.physicalcpu'));
    }
    ...
    public function getCpuLogicCores(): string
    {
        ...
        return trim(shell_exec($this->sysctlPath.'sysctl -n hw.logicalcpu'));
    }
    ...
    public function getMemInfo(): array
    {
        if (PHP_OS == 'Linux') {
            ...
            preg_match('/(\d+)/', shell_exec($this->systemProfilerPath.'system_profiler SPHardwareDataType | grep Memory'), $total);
